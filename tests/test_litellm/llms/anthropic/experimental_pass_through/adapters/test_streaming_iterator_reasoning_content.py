...
def _make_empty_reasoning_warmup_chunk() -> ModelResponseStream:
    """Warmup chunk with reasoning_content present but empty (common on long prompts)."""
    return ModelResponseStream(
        choices=[
            StreamingChoices(
                delta=Delta(
...
def _make_empty_warmup_chunk() -> ModelResponseStream:
    """Warmup chunk with no content fields set."""
    return ModelResponseStream(
        choices=[
            StreamingChoices(
...
class TestEmptyWarmupChunksBeforeThinking:
    """
    Long prompts often emit empty warmup chunks before thinking starts.
    The peek must skip these and open a thinking block, not a spurious text block.
    """

    def test_sync_skips_empty_reasoning_warmup(self):
        chunks = [
            _make_empty_reasoning_warmup_chunk(),
            _make_empty_reasoning_warmup_chunk(),
        ...
        assert not any(
            ...
            and events[events.index(e) + 1].get("type") == "content_block_stop"
            for e in events
            if e.get("type") == "content_block_start"
        ), "Must not emit an empty text block closed before thinking"

    def test_sync_skips_empty_warmup_then_thinks(self):
        chunks = [
            _make_empty_warmup_chunk(),
            _make_empty_warmup_chunk(),
            _make_thinking_chunk("Reasoning."),
        ...
        assert types.count("content_block_start") == 2  # thinking + text only

    @pytest.mark.asyncio
    async def test_async_skips_empty_reasoning_warmup(self):
        chunks = [
            _make_empty_reasoning_warmup_chunk(),
            _make_empty_reasoning_warmup_chunk(),
            _make_thinking_chunk("Let me think..."),
...
class TestBlockTransitionIncludesFirstDelta:
    ...
    and eventually yielded. Without this, the first token of the new block
